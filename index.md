...
* 服务器接收到客户端的请求，开始处理数据，返回给客户端

* 浏览器根据服务器返回的状态码选择后续该如何做，如果是400、500，那么直接报错，如果是300，那么就会重定向、如果是2xx，则next

* 浏览器开始解析服务端返回的数据结构，然后开始渲染，将html渲染成dom树，将css渲染成cssom树，将两者结合生成render tree，然后计算元素在浏览器上出现的位置，然后浏览器绘制
